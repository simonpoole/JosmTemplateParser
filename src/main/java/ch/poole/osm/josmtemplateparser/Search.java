...
public class Search implements Formatter {
    ...
    public Search(@NotNull String searchExpression, @NotNull String template) throws ParseException {
        try {
            condition = (new JosmFilterParser(new ByteArrayInputStream(searchExpression.trim().getBytes())))
                    .condition();
            formatters = (new JosmTemplateParser(new ByteArrayInputStream(template.trim().getBytes())))
                    .formatters();
        } catch (ch.poole.osm.josmfilterparser.ParseException pex) {
            throw new ParseException("Search expression " + searchExpression + " " + pex.getMessage());
        }
    }
